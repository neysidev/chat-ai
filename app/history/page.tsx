      ...
      <div className="py-16">
        ...
        {orderedChats.length === 0 ? (
          <div className="text-center py-12 text-neutral-500">
            ...
            <Link
              href="/"
              className="inline-block mt-4 text-sm font-medium text-neutral-700 hover:underline"
            >
              Go to Home
            </Link>
          </div>
          ...
          <ul className="space-y-2">
            {orderedChats.map(chat => (
              <li key={chat.id}>
                <Link
                  href={`/chat/${chat.id}`}
                  className="block rounded-xl p-4 border border-neutral-200 bg-white transition hover:border-transparent hover:shadow-sm"
                >
